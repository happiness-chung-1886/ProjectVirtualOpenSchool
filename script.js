...
const FAVORITES_KEY = "openLectureHub.favorites";
const THEME_KEY = "openLectureHub.theme";
...
async function initialize() {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`강의 데이터 요청 실패: ${response.status}`);
    ...
    elements.lectureGrid.innerHTML = `
      <div class="empty-state" style="grid-column:1/-1">
        <div>⚠</div>
        <h3>lectures.json을 불러오지 못했어요.</h3>
        <p>파일을 직접 더블클릭하지 말고 로컬 서버나 GitHub Pages에서 열어 주세요.</p>
      </div>`;
    return;
  }
...
function renderCategories() {
  const categories = ["전체", ...new Set(state.lectures.map((lecture) => lecture.category))];
...
function renderLectures() {
  ...
  elements.sectionTitle.textContent = state.favoritesOnly
    ? "저장한 강의"
    : state.category === "전체" ? "전체 강의" : state.category;

  elements.resultSummary.textContent = `${lectures.length}개의 강의`;
...
function createLectureCard(lecture) {
  ...
  image.alt = `${lecture.title} 강의 썸네일`;
  ...
  type.textContent = lecture.playlistId ? `${lecture.type} · Playlist` : lecture.type;
  sourceLink.href = getYouTubeUrl(lecture);

  playButton.setAttribute("aria-label", `${lecture.title} 재생`);
  playButton.addEventListener("click", () => openVideoModal(lecture));

  const isFavorite = state.favorites.has(lecture.id);
  updateFavoriteButton(favoriteButton, isFavorite, lecture.title);
  favoriteButton.addEventListener("click", () => toggleFavorite(lecture.id, favoriteButton, lecture.title));
...
function getVisibleLectures() {
  ...
  const filtered = state.lectures.filter((lecture) => {
    ...
    return (
      (state.category === "전체" || lecture.category === state.category) &&
      (state.level === "all" || lecture.level === state.level) &&
  ...
  return filtered.sort((a, b) => {
    ...
    if (state.sort === "provider") return a.provider.localeCompare(b.provider, "ko");
    if (state.sort === "newest") return String(b.addedAt).localeCompare(String(a.addedAt));
    return Number(b.featured || 0) - Number(a.featured || 0);
  });
}

function toggleFavorite(id, button, title) {
  ...
  updateFavoriteButton(button, state.favorites.has(id), title);

  if (state.favoritesOnly) renderLectures();
}

function updateFavoriteButton(button, isFavorite, title) {
  button.textContent = isFavorite ? "♥" : "♡";
  button.setAttribute("aria-pressed", String(isFavorite));
  button.setAttribute("aria-label", `${title} ${isFavorite ? "저장 해제" : "저장"}`);
}
...
function closeVideoModal() {
  if (elements.videoModal.open) elements.videoModal.close();
}
...
function updateStatistics() {
  ...
  elements.categoryCount.textContent = new Set(state.lectures.map((item) => item.category)).size;
  elements.providerCount.textContent = new Set(state.lectures.map((item) => item.provider)).size;
}
...
function setTheme(theme) {
  ...
  elements.themeToggle.setAttribute("aria-label", theme === "dark" ? "라이트 모드 전환" : "다크 모드 전환");
}

function validateLectures(items) {
  if (!Array.isArray(items)) throw new TypeError("lectures는 배열이어야 합니다.");

  return items
    .filter((item) => item && item.id && item.title && item.provider && item.category && item.videoId)
    .map((item) => ({
      id: String(item.id),
      ...
      level: String(item.level || "입문"),
      language: String(item.language || "영어"),
...
function getYouTubeUrl(lecture) {
  ...
  const playlist = lecture.playlistId ? `&list=${encodeURIComponent(lecture.playlistId)}` : "";
  ...
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="640" height="360">
      ...
      <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle"
        font-family="Arial, sans-serif" font-size="34" fill="#694c86">${escapeSvg(safeText)}</text>
    </svg>`;
...
function saveJSON(key, value) {
  try {
    ...
    console.warn("로컬 저장소에 저장하지 못했습니다.", error);
  }
}
